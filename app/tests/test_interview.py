...
def test_build_active_interview_system_prompt_includes_hr_stage_guidance():
    ...
    assert "Stage focus: probe a specific resume-informed past work example" in prompt
    assert "specific company fact" in prompt
    assert "representative resume-derived example" in prompt
    assert "at least 1-2 questions are grounded in specific resume/profile" in prompt
    assert "You may reference specific resume/profile details" in prompt
